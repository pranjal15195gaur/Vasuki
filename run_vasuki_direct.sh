...
# This script runs a Vasuki file using direct bytecode generation and the C++ VM
...
if [ ! -f "compiler/vm/simple_test" ]; then
    echo "Compiling the VM..."
    cd compiler/vm && make simple_test && cd ../..
fi

# Generate bytecode directly from the AST
python3 compiler/direct_bytecode.py "$INPUT_FILE" "$BYTECODE_FILE" > /dev/null 2>&1
