...
Vasuki supports dynamic scoping through the `dynamic` keyword, which allows variables and functions to be accessed from any context.

#### **Dynamic Variables**

Dynamic variables are declared using the `dynamic var` syntax:

```bash
dynamic var counter = 0;

def increment() {
    counter = counter + 1;  // Accesses the dynamic variable
    return counter;
};
...
Dynamic variables are accessible from any function, regardless of where they were declared.

#### **Dynamic Functions**

Similarly, dynamic functions are declared using the `dynamic def` syntax:

```bash
dynamic def greet(name) {
    print("Hello, " + name + "!");
};

def call_greeter() {
    greet("World");  // Calls the dynamic function
};

call_greeter();  // Output: Hello, World!
```

Dynamic functions can be called from any context, making them useful for utility functions that need to be accessed globally.
