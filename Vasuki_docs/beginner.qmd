...
[Edit on GitHub](https://github.com/YourUser/VasukiDocs/edit/main/beginners.qmd)
...
Vasuki's syntax is designed to be intuitive and beginner-friendly. Here are some of the key elements:

### **Variable Declaration**

Variables in Vasuki are declared using the `var` keyword. For example:
```bash
var x = 10;
```
This creates a variable `x` and assigns it the value `10`.

### **Expressions**

Every expression in Vasuki evaluates to a value. For example:
```bash
var y = x + 5;
```
Here, `y` will hold the value `15`.

### **Blocks**

Blocks of code are enclosed in curly braces `{ }`. For example:
```bash
if (x > 5) {
